...
class Simulation:
    ...
    def dataGeneration(self,numoftask,numofcloudnode,numoffognode):
        ...
        for _ in range(NoOfTask):
            ...
            self.TaskDetails.append([taskInstruction,taskMemory,taskInputSize,taskOutputsize])
        ...
        for Ni in range(TotalNode):
            ...
            for Tk in range(NoOfTask):
                x = round((self.TaskDetails[Tk][0] * (10**3)) / (self.NodeDetails[Ni][0]),4)
                temp7.append(x)
        ...
        for Ni in range(TotalNode):
            ...
            for Tk in range(NoOfTask):
                ...
                bandwidthCost = self.NodeDetails[Ni][3]*(self.TaskDetails[Tk][2]+self.TaskDetails[Tk][3])
    ...
    def calculateMinTotalCost(self):
        #minTotalCost
        costListTranspose =[[row[i] for row in self.costList] for i in range(len(self.costList[0]))]
        minTotalcost = []
    ...
    def utilityFunction(self,makespan,totalcost):
        # alpha = 0.5
        alpha = self.alphaVal
        x= (alpha*(self.minmakespan/makespan)) + ((1-alpha)*(self.minTotalcost/totalcost))
        return x
    ...
    def differential_evolution(self, pop_size, generations, f, cr, bounds):
        ...
        pop = np.random.randint(bounds[0], bounds[1], (pop_size, self.NoOfTask))
        ...
        best_idx = np.argmin([f(self.makeSpan(p),self.totalCost(p)) for p in pop])
        best = pop[best_idx]
